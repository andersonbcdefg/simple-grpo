...
def eval_on_test_set(
    ...
    if args.dataset_type == "correlation":
        ...
        main_error_metric_key_for_return = (
            "metrics/f1_score"  # Or reward, or recall_percent_correct_squares
        )
    else:
    ...
    for batch_idx, batch in enumerate(tqdm(test_loader, desc="Evaluating on test set")):
        ...
        if args.dataset_type == "gui":
            ...
            prompt_to_use = test_loader.prompt  # type: ignore -- Static prompt for clock/correlation
...
def generate_completions(
    ...
    original_prompt_ids: torch.Tensor = prompt_inputs["input_ids"]  # type: ignore

    # Set up generation config
    assert tokenizer.tokenizer is not None and hasattr(
        tokenizer.tokenizer, "pad_token_id"
    ), "tokenizer does not have pad_token_id"
    assert tokenizer.tokenizer is not None and hasattr(
        tokenizer.tokenizer, "eos_token_id"
    ), "tokenizer does not have eos_token_id"
    generation_config = GenerationConfig(
        max_new_tokens=args.max_completion_length,
        do_sample=True,
        temperature=args.temperature,
        pad_token_id=tokenizer.tokenizer.pad_token_id,  # type: ignore
    )

    # Generate all completions at once
    prompt_completion_ids: torch.Tensor = model.generate(  # type: ignore
    ...
    # Do masking
    is_eos = completion_ids == tokenizer.tokenizer.eos_token_id  # type: ignore
    eos_idx = torch.full(
...
def score_completions(
    ...
    rewards_per_func, metrics = eval_class.compute_rewards(
        ...
        completions=cast(evaluator.MessageListBatch, mock_completions_for_eval),
...
def grpo_loss(
    ...
    if args.dataset_type == "gui":
        ...
        current_prompt_text = prompt_info_or_static_prompt["dynamic_prompt"]
        current_answer_data = (
            answer_details_or_string  # This is the dict for CaptchaEvaluator
        )
    else:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="clock",
        choices=["clock", "correlation", "gui", "captcha"],
        help="Type of dataset to use ('clock', 'correlation', 'gui', or 'captcha')",
    )
...
if __name__ == "__main__":
    ...
    for round_num in tqdm(
        ...
        if (
            log_data.get("round_num") == round_num
        ):  # Ensure data is from the correct round
            ...
            if args.dataset_type == "gui":
                ...
                answer_data_for_pdf = log_data[
                    "prompt_info"
                ]  # The dict from PreGeneratedCaptchaLoader
            else:
                prompt_text_for_pdf = log_data[
                    "prompt_info"
            ...
            for compl_idx in range(num_chains_logged):
                ...
                if args.dataset_type == "gui":
                    ...
                    img_path_for_pdf_entry = log_data[
                        "img_path"
                    ]  # Use original image for non-GUI/non-CAPTCHA tasks
