...
from simple_grpo.datasets.gui_generator import IMAGE_WIDTH, IMAGE_HEIGHT  # For max distance calculation
from simple_grpo.typedefs import Message, MessageList, MessageListBatch
    ...
    def _correctness_reward(self, extracted_times: list[str | None], ground_truth_answers: list[str]) -> tuple[list[float], list[float]]:
        ...
        for pred_time_str, true_time_str in zip(extracted_times, ground_truth_answers):
            ...
            if pred_time_obj is None:
                ...
                raise ValueError("this is exactly what we didn't want to happen - true_time_obj is None")
            else:
...
class GUIEvaluator(RewardEvaluator):
    ...
    def compute_rewards(
        self,
        prompts: MessageListBatch | None, # Prompts may not be directly used here but are part of API
        completions: MessageListBatch,
