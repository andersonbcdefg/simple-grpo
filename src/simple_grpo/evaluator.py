...
class RewardEvaluator(ABC):
    ...
    def compute_rewards(
        self,
        prompts: MessageListBatch | None,  # none if prompts not required
        completions: MessageListBatch,
        answers: list[Any],  # surely it stands to reason that this has to be a list?
        device: str,
...
def get_evaluator(name: str) -> RewardEvaluator:
    ...
    if name.lower() == "clock":
        ...
        raise NotImplementedError(
            f"No evaluator implemented for {name}. Supported: 'clock', 'correlation', 'gui', 'captcha'"
        )
...
class ClockEvaluator(RewardEvaluator):
    ...
    def _correctness_reward(
        ...
        for pred_time_str, true_time_str in zip(extracted_times, ground_truth_answers):
            ...
            elif true_time_obj is None:
                raise ValueError(
                    "this is exactly what we didn't want to happen - true_time_obj is None"
                )
            else:
    ...
    def _strict_xml_format_reward(self, completions: MessageListBatch) -> list[float]:
...
class GUIEvaluator(RewardEvaluator):
    ...
    def compute_rewards(
        self,
        prompts: MessageListBatch
        | None,  # Prompts may not be directly used here but are part of API
        completions: MessageListBatch,
...
class CaptchaEvaluator(RewardEvaluator):
    ...
    def __init__(self):
        self.click_call_pattern = re.compile(
            r"click_screen\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)"
        )
        self.strict_xml_pattern = re.compile(
            r"^<reasoning>.*?<answer>.*?</answer>.*?$", re.DOTALL | re.IGNORECASE
        )
        self.answer_tag_content_pattern = re.compile(
    ...
    def _classify_clicks_and_get_stats(
        ...
        false_positives = 0
        true_target_indices = {
            i for i, is_true in enumerate(target_squares_boolean) if is_true
        }
    ...
    def compute_rewards(
        ...
        for i in range(num_completions):
            ...
            xml_format_score = (
                CAPTCHA_XML_FORMAT_REWARD
                if self.strict_xml_pattern.search(completion_text)
                else 0.0
            )
            ...
            stats = self._classify_clicks_and_get_stats(
                predicted_clicks, target_squares_boolean, []
            )
        ...
        metrics = {
            "rewards/f1_score_reward": torch.tensor(
                all_f1_scores, dtype=torch.float32, device=device
            )
            .mean()
            .item()
            if all_f1_scores
            else 0.0,
            "metrics/xml_format_score_avg": np.mean(xml_format_scores_list)
            if xml_format_scores_list
            else 0.0,
            "reward": rewards_per_func.sum(dim=1)
            .mean()
            .item(),  # This will be mean F1 score
            ...
            "metrics/recall_percent_correct_squares": np.mean(all_recalls)
            if all_recalls
            else 0,
            "metrics/f1_score": np.mean(all_f1_scores)
            if all_f1_scores
            else 0,  # Overall F1 for the batch
            "metrics/avg_predicted_clicks": np.mean(all_num_predicted_clicks)
            if all_num_predicted_clicks
            else 0,
        }
        return rewards_per_func, metrics
    ...
    def get_reward_breakdown(self, reward_scores: torch.Tensor) -> dict[str, float]:
        if (
            reward_scores.ndim == 1 and len(reward_scores) == self.num_reward_functions
        ):  # Should be 1
            return {
