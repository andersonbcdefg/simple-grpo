...
class GUIDataLoader(DataLoader):
    ...
    def __next__(self) -> tuple[str, dict[str, Any]]:
        ...
        for _ in range(max_retries):
            gui_image, elements, temp_target_info = (
...
class PreGeneratedCaptchaLoader(DataLoader):
    ...
    def __next__(self) -> tuple[str, dict[str, Any]]:
        ...
        input_image_abs_path = os.path.join(
            self.dataset_root_path, input_image_rel_path
        )
        solution_image_rel_path = metadata["solution_image_path"]
        solution_image_abs_path = os.path.join(
            self.dataset_root_path, solution_image_rel_path
        )
        ...
        dynamic_prompt = CAPTCHA_PROMPT_TEMPLATE.format(
            target_article=article, target_class_name=target_class_name
        )
...
def get_dataloaders(dataset_name: str, **kwargs) -> tuple[DataLoader, DataLoader]:
    ...
    if dataset_name == "clock":
        ...
        captcha_dataset_path = kwargs.get(
            "captcha_dataset_path", "captcha_dataset_output"
        )

        # First, load all metadata file paths
        full_captcha_loader = PreGeneratedCaptchaLoader(
            dataset_root_path=captcha_dataset_path, shuffle=True
        )
        all_metadata_paths = full_captcha_loader.metadata_files[
            :
        ...
        trainloader = PreGeneratedCaptchaLoader(
            dataset_root_path=captcha_dataset_path,
            split="train",
            shuffle=True,
            preloaded_examples=train_metadata_paths,
        )
        testloader = PreGeneratedCaptchaLoader(
            dataset_root_path=captcha_dataset_path,
            split="test",
            shuffle=False,
            preloaded_examples=test_metadata_paths,
        )

        print(
            f"Captcha dataset split: {len(train_metadata_paths)} train, {len(test_metadata_paths)} test examples"
        )
        return trainloader, testloader
    else:
        raise ValueError(
            f"Dataset '{dataset_name}' not supported. Supported: 'clock', 'correlation', 'gui', 'captcha'"
        )
...
if __name__ == "__main__":
    ...
    if os.path.exists("captcha_dataset_output/metadata"):
        try:
            ...
            train_loader_cap, test_loader_cap = get_dataloaders(
                "captcha", captcha_dataset_path="captcha_dataset_output"
            )
            ...
            print(
                f"    Solution Image Path: {answer_data_cap.get('solution_image_path')}"
            )
            print(f"    Target Class: {answer_data_cap['target_class_name']}")
            print(
                f"    Target Squares (first 4): {answer_data_cap['target_squares_boolean'][:4]}..."
            )
            print(
                f"    Dynamic Prompt Snippet: {answer_data_cap['dynamic_prompt'][:150]}..."
            )
            ...
            for i, (img_path_cap, answer_data_cap) in enumerate(test_loader_cap):
                if i >= 2:
                    break  # Only show first 2 test samples
                print(f"    Test Sample {i + 1}:")
                print(f"      Image Path: {img_path_cap}")
                print(
                    f"      Solution Image Path: {answer_data_cap.get('solution_image_path')}"
        ...
        print(
            "Skipping CAPTCHA loader test: 'captcha_dataset_output/metadata' not found."
        )
