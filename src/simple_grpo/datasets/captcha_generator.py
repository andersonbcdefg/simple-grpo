...
CAR_CLASS_ID = 26
MIN_CLASS_PREVALENCE_THRESHOLD = (
    0.01  # Target class should be at least 1% of the (resized) image pixels
)
...
TARGET_CLASSES = {
    k: v
    for k, v in CITYSCAPES_CLASSES.items()
    if v
    in [
        "person",
        "rider",
...
class CaptchaGenerator:
    def __init__(self, num_examples_to_select: int = 100, seed: int = DEFAULT_SEED):
        ...
        if not self.dataset or not isinstance(self.dataset, Dataset):
            raise ValueError(
                "Failed to load dataset from Hugging Face or dataset is empty."
            )
        available_indices = list(range(len(self.dataset)))
        if len(self.dataset) > self.num_examples_to_select:
            self.selected_indices_pool = random.sample(
                available_indices, self.num_examples_to_select
            )
        else:
            self.selected_indices_pool = available_indices
        print(
            f"Initialized CaptchaGenerator with a pool of {len(self.selected_indices_pool)} images from Hugging Face dataset."
        )
        try:
            ...
            try:
                ...
                print(
                    "Warning: DejaVuSans or Arial font not found. Using default PIL font."
                )
    ...
    def _load_hf_dataset(self):
        ...
        try:
            print(
                f"Loading dataset '{HF_DATASET_NAME}' (split: '{HF_DATASET_SPLIT}') from Hugging Face..."
            )
            self.dataset = load_dataset(HF_DATASET_NAME, split=HF_DATASET_SPLIT)  # type: ignore
            assert self.dataset is not None and isinstance(self.dataset, Dataset), (
                "failed to load dataset"
            )
            print(
                f"Successfully loaded {len(self.dataset)} examples from {HF_DATASET_SPLIT} split."
            )
        except Exception as e:
            print(f"Error loading dataset from Hugging Face: {e}")
            self.dataset = None

    def _get_target_class_info(
        self, mask_array: np.ndarray, target_class_id: int
        ...
        if class_pixel_count > 0:
            ...
            if prevalence_ratio >= MIN_CLASS_PREVALENCE_THRESHOLD:
                class_name = CITYSCAPES_CLASSES.get(
                    target_class_id, f"class_id_{target_class_id}"
                )
    ...
    def _draw_banner_content(
        ...
        try:
            ...
            # i believe this is fallback for outdated PIL
            _, line1_h = draw.textsize(line1, font=font_regular)  # type: ignore
            line2_w, line2_h = draw.textsize(line2, font=font_bold)  # type: ignore
    ...
    def _draw_grid_lines(
        ...
        for i in range(1, num_cells_per_side):
            ...
            draw.line(
                [(x, grid_origin_y), (x, grid_origin_y + grid_area_dim)],
                fill=line_color,
                width=line_width,
            )
            # Horizontal lines
            ...
            draw.line(
                [(grid_origin_x, y), (grid_origin_x + grid_area_dim, y)],
                fill=line_color,
                width=line_width,
            )

    def _get_target_squares(
        self, mask_array_resized: np.ndarray, target_class_id: int
    ) -> list[bool]:
        """
        ...
        """
        if (
            mask_array_resized is None or mask_array_resized.ndim != 2
        ):  # Ensure 2D array
        ...
        img_height, img_width = (
            mask_array_resized.shape
        )  # Should be SQUARE_CROP_DIM x SQUARE_CROP_DIM
        if img_height != SQUARE_CROP_DIM or img_width != SQUARE_CROP_DIM:
            print(
                f"Warning: Mask array received by _get_target_squares is not {SQUARE_CROP_DIM}x{SQUARE_CROP_DIM}. Actual: {img_height}x{img_width}"
            )
    ...
    def _draw_solution_markers(
        self,
        image_to_draw_on: Image.Image,
        target_squares: list[bool],
        grid_origin_x: int,
        grid_origin_y: int,
        grid_area_dim: int,
        num_cells_per_side: int,
        ...
        for idx, is_target in enumerate(target_squares):
            if is_target:
                ...
                draw.line(
                    [
                        (cell_x1 + padding, cell_y1 + padding),
                        (cell_x2 - padding, cell_y2 - padding),
                    ],
                    fill=marker_color,
                    width=marker_width,
                )
                draw.line(
                    [
                        (cell_x2 - padding, cell_y1 + padding),
                        (cell_x1 + padding, cell_y2 - padding),
                    ],
                    fill=marker_color,
                    width=marker_width,
                )
    ...
    def generate_captcha_example_for_class(
        self, dataset_idx: int, target_class_id: int, target_class_name_override: str
    ) -> tuple[Image.Image | None, Image.Image | None, str | None, list[bool] | None]:
        """Generates a captcha for a specific class ID using a specific dataset index."""
        example = None
        try:
            assert self.dataset is not None, "self.dataset is None"
            ...
            print(
                f"Error accessing data for example {dataset_idx}. Keys: {keys_available}. Error: {e}"
            )  # Can be noisy
            raise e
            # return None, None, None, None

        resized_image = original_pil_image.resize(
            (SQUARE_CROP_DIM, SQUARE_CROP_DIM), Image.Resampling.LANCZOS
        )
        resized_mask_pil = mask_pil_image.resize(
            (SQUARE_CROP_DIM, SQUARE_CROP_DIM), Image.Resampling.NEAREST
        )
        ...
        prompt_canvas = Image.new(
            "RGB", (prompt_canvas_width, prompt_canvas_height), CANVAS_BACKGROUND_COLOR
        )
        ...
        self._draw_banner_content(
            draw_prompt,
            prompt_canvas_width,
            BANNER_ABS_HEIGHT,
            prompt_text_class,
            self.font_regular,  # type: ignore
            self.font_bold,  # type: ignore
        )
        ...
        padded_prompt_image = Image.new(
            "RGB", (padded_width, padded_height), PADDING_COLOR
        )
        padded_prompt_image.paste(prompt_canvas, (PADDING_SIZE, PADDING_SIZE))

        padded_solution_image = Image.new(
            "RGB", (padded_width, padded_height), PADDING_COLOR
        )
        padded_solution_image.paste(solution_canvas, (PADDING_SIZE, PADDING_SIZE))

        try:
            final_prompt_image = padded_prompt_image.resize(
                (FINAL_DIM, FINAL_DIM), Image.Resampling.LANCZOS
            )
            final_solution_image = padded_solution_image.resize(
                (FINAL_DIM, FINAL_DIM), Image.Resampling.LANCZOS
            )
        except Exception as e:
    ...
    try:
        ...
        if generator.dataset and len(generator.selected_indices_pool) > 0:
            ...
            for class_id_to_try, class_name_to_try in target_classes_to_demo:
                ...
                ):  # Max 2 examples for demo (one of each if possible)
                    break
                ...
                for attempt in range(min(len(indices_to_try), 100)):
                    ...
                    prompt_img, solution_img, target_name, _ = (
                        generator.generate_captcha_example_for_class(
                            idx_from_pool, class_id_to_try, class_name_to_try
                        )
                    )
                    if prompt_img and target_name:
                        prompt_filename = (
                            f"captcha_prompt_DEMO_{target_name.replace(' ', '_')}.png"
                        )
                        solution_filename = (
                            f"captcha_solution_DEMO_{target_name.replace(' ', '_')}.png"
                        )
                ...
                if not found_example_for_this_class:
                    print(
                        f"Could not generate a DEMO {class_name_to_try} CAPTCHA after several attempts."
                    )
        else:
