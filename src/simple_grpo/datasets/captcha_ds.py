...
def calculate_final_target_square_coords(
    target_squares_bool_list: list[bool],
) -> list[list[float]]:
    """
    ...
    for idx, is_target in enumerate(target_squares_bool_list):
        if is_target:
            ...
            final_coords_list.append(
                [final_cell_x1, final_cell_y1, final_cell_x2, final_cell_y2]
            )
...
def generate_captcha_dataset(
    num_items: int = 100,
    output_base_dir: str = "captcha_dataset_output",
    ...
    print(
        f"Initializing CaptchaGenerator with a pool of up to {initial_pool_size} images (seed {seed})..."
    )
    ...
    if not generator.dataset or not generator.selected_indices_pool:
        print("Could not initialize generator with a pool of images.")
        return
    ...
    print(
        f"Attempting to generate {num_items} CAPTCHA examples for 'motorcycle' or 'car'..."
    )
    generated_count = 0
    ...
    random.shuffle(
        indices_to_try
    )  # Use python's random, as generator's np.random is for its internal selections
    ...
    with tqdm(total=num_items, desc="Generating CAPTCHAs") as pbar:
        while generated_count < num_items and image_idx_pointer < len(indices_to_try):
            ...
            current_target_class_id, current_target_class_name = random.choice(
                target_options
            )
            ...
            if (
                prompt_img and target_name_from_gen and target_squares_bools
            ):  # Successful generation for this image and class
                ...
                try:
                    ...
                    final_target_coords = calculate_final_target_square_coords(
                        target_squares_bools
                    )

                    # Convert boolean values to integers (0/1) for JSON serialization
                    target_squares_json_safe = [
                        1 if x else 0 for x in target_squares_bools
                    ]

                    metadata = {
                        "item_id": item_id_str,
                        "input_image_path": os.path.relpath(
                            input_img_path, output_base_dir
                        ),
                        "solution_image_path": os.path.relpath(
                            solution_img_path, output_base_dir
                        ),
                    ...
                    print(
                        f"Error processing or saving item for dataset_idx {dataset_idx_to_try} (target: {target_name_from_gen}): {e}"
                    )
