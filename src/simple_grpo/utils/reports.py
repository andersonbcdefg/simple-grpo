...
def _add_example_header_to_pdf(
    ...
    if os.path.exists(image_path):
        try:
            ...
            if dataset_type == "captcha":
                story.append(
                    Paragraph("<b>Input CAPTCHA Image:</b>", styles["BodyText"])
                )
            ...
            if (
                ...
                and answer_data
                and "solution_image_path" in answer_data
            ):
                ...
                if os.path.exists(solution_image_path):
                    try:
                        story.append(
                            Paragraph(
                                "<b>Ground Truth Solution Image:</b>",
                                styles["BodyText"],
                            )
                        )
                        ...
                        story.append(
                            PlatypusImage(
                                solution_image_path,
                                width=sol_display_width,
                                height=sol_display_height,
                            )
                        )
                        story.append(Spacer(1, 0.1 * inch))
                        ...
                        story.append(
                            Paragraph(
                                f"Error loading solution image: {solution_image_path}. Error: {e}",
                                styles["BodyText"],
                            )
                        )
                else:
                    story.append(
                        Paragraph(
                            f"Solution image not found: {solution_image_path}",
                            styles["BodyText"],
                        )
                    )
    ...
    if dataset_type == "gui":
        ...
        story.append(
            Paragraph(
                f"<b>Target:</b> Select all squares with {article} {target_class_name}",
                styles["BodyText"],
            )
        )
...
def _add_completion_to_pdf(
    ...
    if dataset_type == "captcha" and captcha_data is not None:
        ...
        accuracy_text = [
            "<b>CAPTCHA Accuracy Summary:</b>",
            f"• Model clicked {true_positives} out of {total_targets} correct targets ({true_positives / total_targets * 100:.1f}% recall)"
            if total_targets > 0
            else "• No correct targets to click",
            f"• Model made {total_clicks} total clicks",
        ]

        if false_positives > 0:
            accuracy_text.append(
                f"• Overclicked by {false_positives} (clicked {false_positives} incorrect squares)"
            )
        ...
        if total_clicks > 0:
            ...
            accuracy_text.append(
                f"• Precision: {precision * 100:.1f}% of clicks were on correct targets"
            )
    ...
    if metrics:
        ...
        if total_reward_val is not None:
            metrics_data.append(
                ["Total Reward (this completion)", f"{total_reward_val:.4f}"]
            )
